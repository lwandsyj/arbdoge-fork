...
import "../interfaces/ISwapTools.sol";
import "../interfaces/IDogeTradePool.sol";
import "../pool/Jackpot.sol";
...
contract Jackpot is OwnableUpgradeable {
    ...
    IDogeTradePool public dogeTradePool;
    uint256 public tradePoolMiniAmount;
    ...
    function _addTick(address user, uint256 amount) internal {
        ...
        if (amountUSD >= tradePoolMiniAmount) {
            dogeTradePool.trade(user, amountUSD);
        }
        emit NewTick(
            tickId,
            user,
    ...
    function setDogeTradePool(IDogeTradePool val) public onlyOwner {
        require(address(val) != address(0), "bad val");
        dogeTradePool = val;
    }

    function setTradePoolMiniAmount(uint256 val) public onlyOwner {
        tradePoolMiniAmount = val;
    }
