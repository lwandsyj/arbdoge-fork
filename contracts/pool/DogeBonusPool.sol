...
contract DogeBonusPool is Ownable {
    ...
    function add(
        ...
        require(
            _token != address(0),
            "DogeBonusPool: _token is the zero address"
        );

        require(
            !EnumerableSet.contains(_pairs, _token),
            "DogeBonusPool: _token is already added to the pool"
        );
    ...
    function pendingReward(
        ...
        require(
            ...
            "DogeBonusPool: Can not find this pool"
        );
        ...
        return
            user.amount.mul(accRewardsPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require(
            ...
            "DogeBonusPool: exceed max stake"
        );
    ...
    function _injectRewardsWithTime(
        ...
        uint256 remainingSeconds = bonusEndTime > block.timestamp
            ? bonusEndTime.sub(block.timestamp)
            : 0;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require(
            user.amount >= _amount,
            "DogeBonusPool: Withdraw with insufficient balance"
        );
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyOwner {
        require(
            ...
            "DogeBonusPool: not enough token"
        );
    ...
    function getPoolView(uint256 pid) public view returns (PoolView memory) {
        ...
        uint256 rewardsPerSecond = pool.allocPoint.mul(rewardPerSecond).div(
            totalAllocPoint
        );
        return
            PoolView({
    ...
    function delCaller(address caller) public onlyOwner returns (bool) {
        require(
            caller != address(0),
            "DogeBonusPool: caller is the zero address"
        );
