...
contract CamelotPair is ICamelotPair, UniswapV2ERC20 {
    ...
    function initialize(address _token0, address _token1) external {
        require(
            msg.sender == factory && !initialized,
            "CamelotPair: FORBIDDEN"
        );
        // sufficient check
        token0 = _token0;
        token1 = _token1;

        precisionMultiplier0 = 10 ** uint(IERC20Metadata(_token0).decimals());
        precisionMultiplier1 = 10 ** uint(IERC20Metadata(_token1).decimals());
    ...
    function setFeePercent(
        ...
        require(
            msg.sender == ICamelotFactory(factory).feePercentOwner(),
            "CamelotPair: only factory's feeAmountOwner"
        );
        require(
        ...
        require(
            ...
            "CamelotPair: feePercent mustn't exceed the minimum"
        );
    ...
    function setStableSwap(
        ...
        require(
            msg.sender == ICamelotFactory(factory).setStableOwner(),
            "CamelotPair: only factory's setStableOwner"
        );
    ...
    function setPairTypeImmutable() external lock {
        require(
            msg.sender == ICamelotFactory(factory).owner(),
            "CamelotPair: only factory's owner"
        );
    ...
    function _mintFee(
        ...
        (uint ownerFeeShare, address feeTo) = ICamelotFactory(factory)
            .feeInfo();
    ...
    function burn(
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "CamelotPair: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function _swap(
        ...
        require(
            ...
            "CamelotPair: INSUFFICIENT_OUTPUT_AMOUNT"
        );
...

        {
            require(
                ...
                "CamelotPair: INVALID_TO"
            );
            // optimistically transfer tokens
            if (tokensData.amount0Out > 0)
                _safeTransfer(tokensData.token0, to, tokensData.amount0Out);
            // optimistically transfer tokens
            if (tokensData.amount1Out > 0)
                _safeTransfer(tokensData.token1, to, tokensData.amount1Out);
            if (data.length > 0)
                IUniswapV2Callee(to).uniswapV2Call(
                    msg.sender,
                    tokensData.amount0Out,
                    tokensData.amount1Out,
                    data
                );
            tokensData.balance0 = IERC20(tokensData.token0).balanceOf(
                address(this)
            );
            tokensData.balance1 = IERC20(tokensData.token1).balanceOf(
                address(this)
            );
        }

        uint amount0In = tokensData.balance0 > _reserve0 - tokensData.amount0Out
            ? tokensData.balance0 - (_reserve0 - tokensData.amount0Out)
            : 0;
        uint amount1In = tokensData.balance1 > _reserve1 - tokensData.amount1Out
            ? tokensData.balance1 - (_reserve1 - tokensData.amount1Out)
            : 0;
        require(
            amount0In > 0 || amount1In > 0,
            "CamelotPair: INSUFFICIENT_INPUT_AMOUNT"
        );

        tokensData.remainingFee0 =
            amount0In.mul(_token0FeePercent) /
            FEE_DENOMINATOR;
        tokensData.remainingFee1 =
            amount1In.mul(_token1FeePercent) /
            FEE_DENOMINATOR;

        {
            ...
            uint referrerInputFeeShare = referrer != address(0)
                ? ICamelotFactory(factory).referrersFeeShare(referrer)
                : 0;
            if (referrerInputFeeShare > 0) {
                if (amount0In > 0) {
                    fee =
                        amount0In.mul(referrerInputFeeShare).mul(
                            _token0FeePercent
                        ) /
                        (FEE_DENOMINATOR ** 2);
                    tokensData.remainingFee0 = tokensData.remainingFee0.sub(
                        fee
                    );
                ...
                if (amount1In > 0) {
                    fee =
                        amount1In.mul(referrerInputFeeShare).mul(
                            _token1FeePercent
                        ) /
                        (FEE_DENOMINATOR ** 2);
                    tokensData.remainingFee1 = tokensData.remainingFee1.sub(
                        fee
                    );
            ...
            if (stableSwap) {
                ...
                if (feeTo != address(0)) {
                    ownerFeeShare = FEE_DENOMINATOR
                        .sub(referrerInputFeeShare)
                        .mul(ownerFeeShare);
                    if (amount0In > 0) {
                        fee =
                            amount0In.mul(ownerFeeShare).mul(
                                _token0FeePercent
                            ) /
                            (FEE_DENOMINATOR ** 3);
                        tokensData.remainingFee0 = tokensData.remainingFee0.sub(
                    ...
                    if (amount1In > 0) {
                        fee =
                            amount1In.mul(ownerFeeShare).mul(
                                _token1FeePercent
                            ) /
                            (FEE_DENOMINATOR ** 3);
                        tokensData.remainingFee1 = tokensData.remainingFee1.sub(
        ...
        }
        {
            ...
            uint balance0Adjusted = tokensData.balance0.sub(
                tokensData.remainingFee0
            );
            uint balance1Adjusted = tokensData.balance1.sub(
                tokensData.remainingFee1
            );
            require(
    ...
    function _getAmountOut(
        ...
        if (stableSwap) {
            ...
            return
                (amountIn.mul(reserveB)) /
                (reserveA.mul(FEE_DENOMINATOR).add(amountIn));
        }
    }
    ...
    function sync() external lock {
        ...
        require(
            ...
            "CamelotPair: liquidity ratio not initialized"
        );
